...
export async function fetchStates(country = 'Nigeria') {
  ...
  const response = await fetch(`${API_BASE_URL}/api/hospitals/states?${query.toString()}`);
...
export async function fetchLgas(params = {}) {
  ...
  const response = await fetch(`${API_BASE_URL}/api/hospitals/lgas?${query.toString()}`);
...
export async function fetchHospitals(params = {}) {
  ...
  const response = await fetch(`${API_BASE_URL}/api/hospitals?${query.toString()}`);
...
export async function fetchPlanningSnapshot(params = {}) {
  ...
  if (params.state) query.set('state', params.state);
  if (params.lga) query.set('lga', params.lga);
  const response = await fetch(`${API_BASE_URL}/api/hospitals/planning/deserts?${query.toString()}`);
...
export async function verifyPayment(txnRef) {
  const response = await fetch(`${API_BASE_URL}/api/payments/verify/${txnRef}`);
...
export async function simulatePayment(txnRef, result = 'success') {
  const response = await fetch(
    `${API_BASE_URL}/api/payments/simulate/${txnRef}?result=${encodeURIComponent(result)}`,
    { method: 'POST' }
  );
