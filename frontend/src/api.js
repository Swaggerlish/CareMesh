...
export async function fetchStates(country = 'Nigeria') {
  ...
  const response = await fetch(`${API_BASE_URL}/hospitals/states?${query.toString()}`)
...
export async function fetchLgas(params = {}) {
  ...
  const response = await fetch(`${API_BASE_URL}/hospitals/lgas?${query.toString()}`)
...
export async function fetchHospitals(params = {}) {
  ...
  const response = await fetch(`${API_BASE_URL}/hospitals?${query.toString()}`)
...
export async function fetchPlanningSnapshot(params = {}) {
  ...
  if (params.state) query.set('state', params.state)
  if (params.lga) query.set('lga', params.lga)
  const response = await fetch(`${API_BASE_URL}/hospitals/planning/deserts?${query.toString()}`)
...
export async function verifyPayment(txnRef) {
  const response = await fetch(`${API_BASE_URL}/payments/verify/${txnRef}`)
...
export async function simulatePayment(txnRef, result = 'success') {
  const response = await fetch(`${API_BASE_URL}/payments/simulate/${txnRef}?result=${encodeURIComponent(result)}`, {
    method: 'POST',
  })
...
export async function sendChat(message, state, lga) {
  const response = await fetch(`${API_BASE_URL}/chat/`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ message, hospital_state: state, hospital_lga: lga }),
  })
